...
export function FormatMessage(cartMessage: IOrderProducts[]) {
   ...
   const formatedMessage = cartMessage.map((product, index) => {
      return `${product.productQuantity}x - ${product.productName}/  ${product.options?.length > 0 && "Adicionais: /" + product.options?.map((item) => "   " + item.optionName + " " + BRLReais.format(item.optionPrice).replace(",", ".") + "/")}  Subtotal: ${BRLReais.format(product.productOriginalPrice).replace(",", ".")}//`
